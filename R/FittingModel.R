...
chooseModel <- function(dat,
  ...
  setorder(dat,DilutionPoint)[!is.na(get(tidyselect::all_of(y)))]
  dat$outlier <- FALSE
  ...
  if ("linear" %in% ModelName) {ModelName = "linear"} else if (all(c("logistic", "quadratic") %in% ModelName)) {ModelName = "logistic"}  # if same correlation
  Model <- ModelName

  Model = list("fit" = fitted(get(Model)), "coefficients" = coef(get(Model)), "residuals" = residuals(get(Model)))
