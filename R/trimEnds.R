...
trimEnds <- function(dats, y="YLog", x="XLog", thresh=0){
  ...
  if (last(dat[[tidyselect::all_of(y)]]) != max(dat[[tidyselect::all_of(y)]])) {
    dat.reduced.max <- copy(dat)[
      , Comment := as.character(Comment)][
        get(all_of(y)) >= last(get(tidyselect::all_of(y))) - thresh,
  ...
  if (dat[[tidyselect::all_of(y)]][1] != min(dat[[tidyselect::all_of(y)]])){
    ...
    dat.reduced.min <- copy(dat)[
      , Comment := as.character(Comment)][get(tidyselect::all_of(y)) <= get(tidyselect::all_of(y))[1] + thresh,
                            ':=' (color = "grey", trim = TRUE,
  ...
  dats[IDintern %in% tmp$IDintern, ':=' (pch = tmp$pch, color = tmp$color, trim = ifelse(is.null(tmp$trim), FALSE, tmp$trim))]
