...
library(assertthat)
library(pbapply)
opts_chunk$set(fig.width=7, fig.height=7, fig.path='C:/Users/Wiebachj/OneDrive - Charité - Universitätsmedizin Berlin/Kirwan/sigmodial/sigmodial/scripts/img/',
...
# Args: column names,file, residuals for outlier detection, minimal consecutive points in linear range, minimal fitting correlation 
  ...
  processList$dataRaw <- processList$dataOrigin %>% 
    rename(ID = columnNames[["ID"]],
           Batch = columnNames[["Batch"]], 
           Replicate = columnNames[["Replicate"]], 
           mz = columnNames[["MZ"]], 
           RT = columnNames[["RT"]], 
  ...
  ## outlier
  dataOut <- lapply(unique(processList$dataPrep$ID),
...
discardCompound <- n_distinct(processList$dataTrim %>%  filter(enoughPoints == FALSE) %>% select(ID))
remainingCompound <- n_distinct(processList$dataTrim %>%  filter(enoughPoints != FALSE) %>% select(ID))
...
discardCompoundFitting <- n_distinct(processList$dataFittingModel %>%  filter(aboveMinCor == FALSE) %>% select(ID))
remainingCompoundFitting <- n_distinct(processList$dataFittingModel %>%  filter(aboveMinCor == TRUE) %>% select(ID))
...
savedCompounds <- n_distinct(processList$dataFittingModelSOD %>% filter(aboveMinCor.second == TRUE) %>% select(ID))
...
processList$dataFittingModelAll <- bind_rows(processList$dataFittingModel %>%  filter(aboveMinCor == TRUE),
                                             processList$dataFittingModelSOD %>%  filter(aboveMinCor.second == TRUE) %>% 
                                               ...
                                               select(-contains(c("first", "second"))), .id = NULL
                                             )
...
processList$fittingModelAll <- c(processList$fittingModel[names(processList$fittingModel) %in% unlist(distinct(processList$dataFittingModel %>% filter(aboveMinCor == TRUE) %>% select(ID)))],
                                 processList$fittingModelSOD[names(processList$fittingModelSOD) %in% unlist(distinct(processList$dataFittingModelSOD %>% filter(aboveMinCor.second == TRUE) %>% select(ID)))])
...
getSummaryListOverview <- function(completeList){
  #browser()
 dat <- completeList[["dataRaw"]] %>% 
    select(ID, Batch, Replicate, mz, RT) %>% 
    distinct(.)
 
 LR <- completeList[["Preprocessed"]][["dataLinearRange"]] %>% 
   select(ID, linearRangeStart, linearRangeEnd) %>% 
   distinct(.) %>%
 ...
 LR_limits <-  lapply(LR$ID, function(x) {completeList[["dataPrep"]] %>% 
   ...
   select(ID, LR_ConcentrationStart, LR_ConcentrationEnd, LR_IntensityStart, LR_IntensityEnd) %>% 
   distinct()
 }) %>% ldply
  ...
  MissingNr <- completeList[["dataRaw"]] %>%
    select(ID, Intensity) %>% 
    group_by(ID) %>% 
    count(is.na(Intensity)) %>%
    rename(Nr_MissingValue = n) %>%
    filter(`is.na(Intensity)` == TRUE) %>% 
    select(ID, Nr_MissingValue)
  ...
  OutlierNr <- completeList[["dataOut"]] %>% 
    filter(Outlier %in% TRUE) %>% 
    group_by(ID) %>% 
    count(Outlier) %>% 
    rename(Nr_Outlier = n) %>% 
    select(ID, Nr_Outlier)
  
  Outlier <- lapply(LR$ID, function(x){
    completeList[["dataOut"]] %>%
    filter(ID %in% x) %>%
    filter(row_number() %in% LR$linearRangeStart[LR$ID %in% x]: LR$linearRangeEnd[LR$ID %in% x]) %>% 
      summarise(ID, "OutlierInLinearRange" = any(Outlier %in% TRUE)) %>% 
      distinct()
  }) %>% ldply  
...
getAllList <- function(completeList){
  ...
  dat <- left_join(
    x = dat,
    y = completeList$Preprocessed$dataLinearRange %>% select(ID, DilutionPoint, linearRange),
    by = c("ID", "DilutionPoint")
  ) %>% 
    arrange(ID, DilutionPoint)
...
func_hist_QC_summary <- function(datList = processList$SummaryAll){
  
  NAdat <- datList$DilutionPoint[datlist$MissingValue == TRUE] %>% table()
  ...
  summary_dat <- tibble(
    ...
    "Nr of non valid Points" = rep(length(unique(datList$ID)), length(unique(datList$DilutionPoint))) - ValidNr
  )
...
# required columns in csv: ID, Batch, Replicate, MZ, RT, Concentration, Intensity
...
# processList$dataRaw <- processList$dataOrigin %>% 
#   rename(ID = columnNames$ID, Batch = columnNames$Batch, Replicate = columnNames$Replicate, mz = columnNames$MZ, RT = columnNames$RT, Concentration = columnNames$Concentration, Intensity = columnNames$Intensity) %>% 
#   filter(Replicate %in% 1)
# 
# processList$dataPrep <- processList$dataRaw  %>% prepareData(dat = .)
