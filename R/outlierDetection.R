...
outlierDetection <- function(dat, y= Intensity, x= Concentration,
                             ...
                             R2FOD=0.99, numboutlier = 1){
  #browser()
  #dat <- as_tibble(dat) #%>% arrange(DilutionPoint)
  setorder(dat,DilutionPoint)
  dataOutlier <- dat %>% tidyr::drop_na(tidyselect::all_of(y))
  #dataOutlier$Outlier <- NA
  bestModel <- chooseModel(dat, tidyselect::all_of(y), tidyselect::all_of(x), model)

  dataModel <- tibble::tibble(
  ...
  if (dataModel$correlationModel < R2FOD) {
    dataOutlier <- outlier(dat = dataOutlier,modelObject =  bestModel[[1]][[2]], res = SRES, count = numboutlier)
  ...
  Outlier <- dplyr::full_join(dataOutlier, dataModel, by = "groupIndices") |> dplyr::select(IDintern, groupIndices,Comment,outlier,pch, color, residuals, ModelFit, correlationModel )
