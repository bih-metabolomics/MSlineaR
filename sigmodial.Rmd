...
process.list <- list( data_raw = data_tbl_raw )
...
#name = 'FEAT_279_232__57_1146'#'FEAT_245_048__371_345'
...
dat <- data_tbl_work1 %>% 
  #drop_na() %>% 
  group_by(ID) %>%  
  filter(Replicate == 1) %>% #, ID %in% c(sample(metabolites, 25), name)) %>%  
  mutate(Concentration_raw = Concentration, Concentration_log = log(Concentration)) %>%
  mutate(DilutionPoint = row_number(), Comment = "", Intensity_raw = Intensity, Intensity_norm = Intensity/max(Intensity, na.rm = T)*100, DP = DilutionPoint-mean(DilutionPoint))

process.list$data_prep = dat

metabolites <- unique(data_tbl_work1$ID)

dat %>% 
ggplot(aes(DP, Intensity_norm)) +
  geom_point() + 
  #geom_line() + 
   #geom_line(aes(DP, Intensity.norm)) +
  theme_bw() +
  #geom_smooth(method = lm) +
   #stat_smooth(method = "lm", formula = y~poly(x,2), aes(log_conc, reg2$fitted.values, col = "Order 1")) +
  #scale_y_continuous(limits = c(0, 30), breaks = seq(0, 12, by = 1)) +
...
plot(y = dats$Intensity_norm,x = dats$DP)
...
```{r find outlier}
## fitting
name = 'FEAT_159_051__251_569'#'FEAT_117_07__28_9557'#'FEAT_133_097__482_438'#c('FEAT_146_081__71_7346', 'FEAT_117_07__28_9557', 'FEAT_118_061__394_271')

data.input <- dat %>% filter(ID %in% name)
dats = data.input

library(drc)
out <- function(dats, res = 2){
...
a <- drm(Intensity_norm ~ DP, fct = L.3(), data = dats) #model.sigmoid
b <- lm(Intensity_norm ~ DP, data = dats) #model.linear
c <- lm(Intensity_norm ~ poly(DP,2,raw=TRUE), data = dats) #model.quadratic


# lines(y = predict(a), x = dat$DP,col = "green")
# lines(y = predict(b), x = dat$DP, col = "red")
# lines(y = predict(c), x = dat$DP, col = "blue")



# calculate correlation
cor.poly <- c(
  cor(dats$Intensity_norm, predict(a)),
  cor(dats$Intensity_norm, predict(b)),
  cor(dats$Intensity, predict(c))
)

cor.max <-  which(cor.poly == max(cor.poly))
cor.max <- get(chartr("123", "abc",cor.max))

SSE <- sum((fitted(cor.max) - dats$Intensity_norm)^2)

# plot fitted curve
#lines(y = fitted(cor.max), x = dats$DP, col = "green")

# residuals
if(max(cor.poly) < 0.99) {
  residual <- abs(residuals(cor.max)/sd(residuals(cor.max)))
  if(max(residual) > res){rs <- which(residual == max(residual))}

#points( y = dat$Intensity_norm[rs], x = dat$DP[rs], col = "red", pch = 20)
...
data.out <- lapply(unique(dat$ID)[1:100], function(x) out(dat %>% filter(ID %in% x))) %>% ldply %>%  distinct()


#data.out <- out(data.input)
...
```{r function trimming}
trimCurve <- function(dat, Intensity.name = "Intensity_norm", thresh = 0){
  
  if(last(dat[[Intensity.name]]) != max(dat[[Intensity.name]])){  
    dat.reduced.max <- dat %>% 
      filter(get(Intensity.name) >= last(get(Intensity.name)) - thresh) %>% 
      mutate(Comment = "bigger") %>% 
      select(DilutionPoint, Comment)
  } else {dat.reduced.max <- dat %>% select(DilutionPoint, Comment)}
    
  if(dat[[Intensity.name]][1] != min(dat[[Intensity.name]])){
    dat.reduced.min <- dat %>% 
      filter(get(Intensity.name) <= get(Intensity.name)[1] + thresh) %>% 
...
process.list$data.trimmed <- lapply(unique(data.out$ID), function(x) {trimCurve(data.out %>% filter(color != "red", ID %in% x))}) 

trim.dat <- process.list$data.trimmed %>% ldply %>%  distinct()

trim_out <- left_join(data.out %>% select(-Comment), trim.dat, )
trim_out$color[trim_out$Comment %in% c("bigger", "smaller")] <- "blue"
trim_out$pch[trim_out$Comment %in% c("bigger", "smaller")] <- 19
...
slopes <- slide(2 : nrow(dat), ~ slope(y = dat$Intensity_norm, x = dat$DP, index = .x)) %>% ldply %>% mutate(Nr = 2:nrow(dat), comment = "")



## fitting
library(drc)
model.sigmoid <- drm(Intensity_norm ~ DP, fct = L.3(), data = dat)
model.linear <- lm(Intensity_norm ~ DP, data = dat)
model.quadratic <- lm(Intensity_norm ~ poly(DP,2,raw=TRUE), data = dat)


lines(y = predict(model.sigmoid), x = dat$DP,col = "green")
lines(y = predict(model.linear), x = dat$DP, col = "red")
lines(y = predict(model.quadratic), x = dat$DP, col = "blue")

which(abs(residuals(model.sigmoid)/sd(residuals(model.sigmoid))) > 2)

cor(dat$Intensity_norm, predict(model.sigmoid))
cor(dat$Intensity_norm, predict(model.linear))
cor(dat$Intensity, predict(model.quadratic))
