...
#                    x = 1:length(y))
data_tbl_raw2 <- read.csv("C:/Users/wiebachj/OneDrive - Charité - Universitätsmedizin Berlin/Kirwan/20210525_Raw Data.csv", header = T, check.names = F, blank.lines.skip = TRUE)
...
```{r data chunk, results ='hide'}

#generate result table template
...
data_tbl_complete <- data_tbl %>% group_by(ID) %>% filter(sum(!is.na(Intensity)) > 5) 
# 559 Signal  series were removed -> 221 groups are left
...
slider_func_lm <- function(tbl, x){
  ...
  lm_sum <- lm(Intensity ~ Concentration, data = tbl[x : (x + 2),]) %>%
    summary()
  alpha.rad <- atan((data$Intensity[x + 2] - data$Intensity[x])/(data$Concentration[x + 2] - data$Concentration[x]))
...
slider_func_group <- function(dat, metabolite, points, adjr2 = 0.9, degr = 10){
  tbl_linear_Range <- list()
...
slope_x <- 1.2
data_c <- tibble(
  y_raw = c(1,1,1,1, c(1:7 * slope_x),9,9,9,9),
...
dataWithrange <- lapply(unique(data_c$ID),function(x) {slider_func_group(dat = data_c, metabolite = x, points = 5, adjr2 = 0.85, degr = 10)}) %>% unlist(., recursive = F) %>% ldply
