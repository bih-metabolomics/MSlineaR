...
func_hist_QC_summary <- function(datlist = dataWithrangeAll){
  ...
  ValidDat <- dataPlot$DilutionPoint[dataPlot$Comment == "linear"] %>% table()
  ...
  summary_dat <- tibble(
    ...
    "Nr of non linear Points" = rep(Compdat, length(unique(dataPlot$DilutionPoint))) - NANr - OutNr - ValidNr
  )
...
func_plot_linearRange(datlist = dataWithrangeAll, IDs = "metabolite out4")
...
int50 <- DescTools::Closest(x = dat$Intensity,a = max(fitted(d))/2, which = TRUE)


#create linear regression line going through int50
linear.range <- lm(fitted(d)[(int50 - 1) : (int50 + 1)] ~ dat$log_conc[(int50 - 1) : (int50 + 1)])
...
ndx <- which(abs((abline.intensity - fitted(c))/max(abs(abline.intensity - fitted(c)))) < 0.5)
