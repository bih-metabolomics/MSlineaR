...
func_slider_LinearRange <- function(dat, metabolite, points, adjR2 = 0.9, degr = 10, cooksdis = 4){
  ...
  if (any(linRange$Comment == "")) { # found one linear range, calculate slope and R2 for the whole range
    ...
    linRange$Comment <- ""
    linRange$OutlierOrNAPos <- NA
    linRange$OutlierOrNAinRange <- NA
    linRange$type <- "complete Case"
...
func_hist_summary <- function(dat = dataWithrange){
  ...
  summary_dat <- tibble(
    "Dilution Points" = c(1:15),
    "NAs" = NANr,
    "Outliers" = OutNr,
    "Valids" = rep(Compdat, 15) - NANr - OutNr
  )
...
# data_c %>% 
# mutate(across(type, factor, levels=c("smooth","outlier_high","outlier_low", "NA"))) %>%
...
# ,labeller = as_labeller(c("smooth" = "metabolite1 - smooth", "outlier_high" = "metabolite2 - outlier_high","outlier_low" = "metabolite3 - outlier_low", "NA"= "metabolite4 - missing Values")))
...
#data_calc <- data_c %>% filter(type == "NA")
dataWithrange <- lapply(unique(data_c$ID),function(x) {func_slider_LinearRange(dat = data_c, metabolite = x, points = 5, adjR2 = 0.85, degr = 10)}) %>% unlist(recursive = F) %>% ldply %>%  distinct()

dataWithrange

meta = "metabolite1"

data_plot <- data_c %>% filter( ID == meta) %>% mutate(color = "black", size = 2)
data_plot$color[dataWithrange[dataWithrange$ID == meta, c("start")]] <- "green"
data_plot$size[dataWithrange[dataWithrange$ID == meta, c("start")]] <- 4
data_plot$color[dataWithrange[dataWithrange$ID == meta, c("end")]] <- "green"
data_plot$size[dataWithrange[dataWithrange$ID == meta, c("end")]] <- 4
